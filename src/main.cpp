...
#if defined(NDEBUG)
# error "Bitcoinex cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x32f09e662bfd28f59606b369aefd8a85891ee4ab82a85f2aa163e786f773eab9");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Bitcoinex: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Bitcoinex Signed Message:\n";
...
{
    // Bitcoinex: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    if(nHeight == 1)
    {
    	nSubsidy = 10000000 * COIN; // 10%  for development, promotion, airdrops and bounties, exchange services
    }
    else if(nHeight <= 800000)
    {
        nSubsidy = 50 * COIN; // 50 BTCe : Until coin supply reaches 50,000,000
    }
    else if(nHeight <= 2400000) 
    {
        nSubsidy = 25 * COIN; // 25 BTCe : Until coin supply reaches 90,000,000
    }
    else if(nHeight <= 3300000) 
    {
        nSubsidy = 10 * COIN; // 10 BTCe : Until coin supply reaches 99,000,000
    }
...
static const int64 nTargetTimespan = 360; // bitcoinex: 6 minutes
static const int64 nTargetSpacing  = 60; // bitcoinex : 1 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Bitcoinex: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Bitcoinex: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x32f09e662bfd28f59606b369aefd8a85891ee4ab82a85f2aa163e786f773eab9");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "22/oct/2017 drink coffee at home";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0xbdb459f5777d60a0a08cb6319474fbbf3458196c9a046c04cfe5b8d78536f9cb"));
...
unsigned char pchMessageStart[4] = { 0xc3, 0xd2, 0xd1, 0xbd }; // Bitcoinex: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BitcoinexMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BitcoinexMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BitcoinexMiner(CWallet *pwallet)
{
    printf("BitcoinexMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("Bitcoinex-miner");
    ...
    try { loop {
        ...
        printf("Running BitcoinexMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BitcoinexMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BitcoinexMiner, pwallet));
}
